...
- [~] แปลง empty-catch → error state (opportunistic) — ✅ 2026-09-01 (admin.html): helper `_loadFail(e,label)` — loader หลักของหน้า settings 12 ตัว (appearance/shopdecor/icons/content/features/store/courier/shipping/points/streak/promo/platformRoots + brand switcher) → console.error + toast เตือน "โหลด...ไม่สำเร็จ" (กันแอดมินกดบันทึกทับค่า default ที่โหลดไม่ขึ้น) · sub-loader/boot/best-effort 6 ตัว → log เงียบ · **คงเดิม** (ตั้งใจ): count-in-loop, `_collCount` fallback, brand-name, claim cleanup, print, localStorage · **index.html:** ✅ 2026-09-01 ตรวจแล้ว — §3 compliant อยู่แล้ว (empty-catch ที่เหลือเป็น best-effort จริง: fallback/graceful-degrade/มี err-callback เช่น onSnapshot→console.warn, deep-link→toast, signInAnon→fallback ชื่อ) → **ไม่แตะ** (ใส่ toast = error ปลอม = regression)
- [x] ~~dedup login-provider buttons (ARCHITECTURE §9 copy-paste)~~ — ✅ 2026-09-01: `_applyProviderButtons(ids)` รวมบล็อกจัดปุ่ม Google/FB+divider+hint ที่ซ้ำเป๊ะใน `showLoginPrompt`(gate)/`showLoginScreen`(full) → เหลือเรียก helper บรรทัดเดียว (behavior เท่าเดิม · guard 9/9 · index parse 3/3 · frontend 33/33)
- [x] ~~เก็บ dead code (ARCHITECTURE §9)~~ — ✅ 2026-09-01: ของเก่าใน §9 (`likePost`/`ADMIN_LINE_ID`/`public/`/`apply_*.js`/config samples) **ถูกเก็บไปก่อนแล้ว** · สแกนหา dead จริงปัจจุบัน (substring-count ≤1 + grep ยืนยัน) ลบ **-114 บรรทัด**: index.html `renderAdminBranding`+`saveBranding` (in-app branding editor เก่า→ย้ายหลังบ้าน · transitive dead), `renderProfileOrders` (→openMyOrders modal), `toggleObCrop` (onboarding crop), `uploadCoverPhoto` (เอา cover ออกแล้ว) · admin.html `syncColor` (N7 ตัดสีออก) · re-scan: index.html dead=0 · **เก็บไว้ตั้งใจ:** `setCurrentTenant` (setter 1 บรรทัด มี doc — intended API ไม่ใช่ dead feature) · guard 9/9 · parse 3+1 · frontend 33/33
...
- [x] ~~domain แบรนด์ที่ไม่จดเอง (subdomain/path)~~ — ✅ 2026-08-28: **tenant-resolution trilogy** ครบใน `tenantId()` — ?t= > hostname(BYOD) > **subdomain `<sub>.<root>`** (มาตรฐานสากล Shopee/Shopify · pure `subdomainTid` · root จาก `platform/domains.roots` ที่ super ตั้งในหน้า "จัดการแบรนด์" · **domain-agnostic ไม่ hardcode** · inert จนกว่าตั้ง) > path `bocean.com/brandx` > default · frontend test (subdomain/path/reserved · 29/29) · [[custom-domain-byod]]
  - **เหลือ Roger (infra ครั้งเดียว):** wildcard `*.<root>` DNS + wildcard SSL (GCP Load Balancer managed cert หรือ Cloudflare) — Firebase Hosting auto-wildcard ไม่ได้ → นี่คือตัวที่ทำให้ subdomain auto ทุกร้าน

...
- ✅ **preflight validation ขนส่ง (2026-08-25):** ก่อนยิง API จริงใน `createShipment` เช็กข้อมูลครบ — ผู้รับ (order.shipping: ชื่อ/เบอร์/ที่อยู่/ตำบล/อำเภอ/จังหวัด/รหัส ปณ. 5 หลัก) + ผู้ส่ง (settings/store เหมือนกัน) + น้ำหนัก (order.weight>0) · ขาด → `failed-precondition` บอกรายการที่ขาด · โหมด mock ข้าม preflight (เทสต์ flow ได้) · helper `shipmentPreflight` (export `_shipmentPreflight`) · e2e courier +5 (11/11, รวม 70/70) · deploy `createShipment` · **เตรียมไว้ให้วันเสียบ Shippop key ลื่น** · **Shippop = provider ที่เลือก** (aggregator, ฟรีจนกว่าจะส่งจริง, รายได้จากส่วนต่างค่าส่ง+COD 3%/2.5%) · **รอ Roger สมัคร developers.shippop.com ขอ sandbox key**

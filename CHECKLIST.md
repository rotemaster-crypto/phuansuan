...
- ✅ **ใบส่งสินค้า + user แก้ที่อยู่ (2026-08-25):** admin ปุ่ม "พิมพ์ใบส่งสินค้า" ต่อออเดอร์ (ดึง `order.shipping` = ที่อยู่ที่ user กรอก → หน้าพิมพ์ผู้ส่ง/ผู้รับ/รายการ) · user แก้ที่อยู่จัดส่งบนออเดอร์ที่ยังไม่ส่งได้ (modal → อัปเดต order.shipping + ที่อยู่เริ่มต้น) · rules order update เพิ่มให้เจ้าของแก้ `shipping` ได้ตอนสถานะ pending/paid_review/confirmed
- ▶️ **ถัดไป:** dogfood/pilot · ปรับแต่ง UX ตามที่ Roger เทสต์เจอ
